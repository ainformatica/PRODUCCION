...
switch ($_GET["op"]){
	...
	if (empty($id_actividad_voae)){
		...
		$rspta=$actividad->insertar($nombre_actividad,$ubicacion,$fch_inicial_actividad,$fch_final_actividad,$descripcion,$poblacion_objetivo,$presupuesto,$staff_alumnos,$observaciones,$usuario,$id_ambito,$periodo);
		...
		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'INSERTO', 'LA ACTIVIDAD: "' . $nombre_actividad . '"');
				...
				$valor = "select nombre_actividad, ubicacion , fch_inicial_actividad, fch_final_actividad, descripcion , poblacion_objetivo, presupuesto,staff_alumnos, observaciones , id_ambito, periodo from tbl_voae_actividades WHERE id_actividad_voae= '$id_actividad_voae'";
				$result_valor = $mysqli->query($valor);
				$valor_viejo = $result_valor->fetch_array(MYSQLI_ASSOC);

				//CONDICION PARA LA MODIFICACION DEL NOMBRE Y LA DESCRIPCION
				if ($valor_viejo['nombre_actividad'] <> $nombre_actividad and $valor_viejo['ubicacion'] <> $ubicacion and $valor_viejo['fch_inicial_actividad'] <> $fch_inicial_actividad and $valor_viejo['fch_final_actividad'] <> $fch_final_actividad and $valor_viejo['descripcion'] <> $descripcion and $valor_viejo['poblacion_objetivo'] <> $poblacion_objetivo and $valor_viejo['presupuesto'] <> $presupuesto and $valor_viejo['staff_alumnos'] <> $staff_alumnos and $valor_viejo['observaciones'] <> $observaciones and $valor_viejo['id_ambito'] <> $id_ambito and $valor_viejo['periodo'] <> $periodo) {
					bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'MODIFICO LA ACTIVIDAD  CON EL NO DE SOLICITUD', '"' . $id_actividad_voae . '"');

					$rspta=$actividad->editar($id_actividad_voae,$nombre_actividad,$ubicacion,$fch_inicial_actividad,$fch_final_actividad,$descripcion,$poblacion_objetivo,$presupuesto,$staff_alumnos,$observaciones,$id_ambito,$periodo);
		echo $rspta ? "ACTIVIDAD ACTUALIZADA" : "ACTIVIDAD NO SE PUEDE MODIFICAR";
		...
		case 'insertaryenviar':
		if (empty($id_actividad_voae)){
				//SE MANDA A LA BITACORA LA ACCION DE INSERTAR
		$rspta=$actividad->insertaryenviar($nombre_actividad,$ubicacion,$fch_inicial_actividad,$fch_final_actividad,$descripcion,$poblacion_objetivo,$presupuesto,$staff_alumnos,$observaciones,$usuario,$id_ambito,$periodo);
		echo $rspta ? "ACTIVIDAD REGISTRADA" : "actividad no se pudo registrar";
		
		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'INSERTO', 'LA ACTIVIDAD: "' . $nombre_actividad . '"');
		}else {
		$rspta=$actividad->insertaryenviar2($id_actividad_voae,$nombre_actividad,$ubicacion,$fch_inicial_actividad,$fch_final_actividad,$descripcion,$poblacion_objetivo,$presupuesto,$staff_alumnos,$observaciones,$usuario,$id_ambito,$periodo);
		echo $rspta ? "ACTIVIDAD REGISTRADA" : "actividad no se pudo registrar";
		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'INSERTO', 'LA ACTIVIDAD: "' . $nombre_actividad . '"');
			}
		break;
	    ...
	    $bt_no_solicitud = $result_valor->fetch_array(MYSQLI_ASSOC);
 		...
 		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'ENVIO', 'EL No DE SOLICITUD "' . $bt_no_solicitud['id_actividad_voae'] . '"');
		break;
	    ...
	    while ($reg=$rspta->fetch_object()){
	    	$data[]=array(
	    		"0"=>($reg->denegacion)?'<button id="btn_editar" class="btn btn-warning" '.$_SESSION["btn_editar"].' onclick="mostrar('.$reg->id_actividad_voae.')"><i class="far fa-edit"></i></button>'. 
	    		...
	    		'<button id="btn_editar" class="btn btn-warning" '.$_SESSION["btn_editar"].' onclick="mostrar('.$reg->id_actividad_voae.')"><i class="far fa-edit"></i></button>'.
	    		'<button id="btn_dene"title="Ver Motivo Denegación " class="btn btn-danger pull-right"  '.$_SESSION["btn_dene"].' onclick="mostrar2('.$reg->id_actividad_voae.',1)"><i class="fa fa-info"></i></button>',
