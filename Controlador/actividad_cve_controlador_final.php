...
switch ($_GET["op"]){
	...
	case 'cancelar':
	    ...
	    $estado = $result_valor->fetch_array(MYSQLI_ASSOC);
 		...
 		$rspta=$actividad->cancelar($id_actividad,$just_act);
 		echo $rspta ? "Actividad Cancelada" : "La actividad no se puede cancelar";
 		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'CANCELO', 'LA ACTIVIDAD CON EL NO DE SOLICITUD "' . $estado['id_actividad_voae'] . '"');
	...
	break;
	    ...
	    $bt_no_solicitud = $result_valor->fetch_array(MYSQLI_ASSOC);
 		...
 		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'FINALIZO', 'EL NO DE SOLICITUD "' . $bt_no_solicitud['id_actividad_voae'] . '"');
	    break;
	    ...
	    while ($reg=$rspta->fetch_object()){
	    	$data[]=array(
	    		...
	    		' <button class="btn btn-danger" title="Cancelar Actividad" onclick="mostrar2('.$reg->id_actividad_voae.')"><i class="far fa-window-close"></i></button>'.
